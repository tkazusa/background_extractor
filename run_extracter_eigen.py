...
from extracter_eigen import BackGroundExtracterEigen
...
class RunnerBackGroundExtracterEigen(RunnerBase):
    ...
    def _set_extracter(self, height, width):
        """Set extracter.
        """
        return BackGroundExtracterEigen(height, width)
...
if __name__ == "__main__":
    ...
    runner = RunnerBackGroundExtracterEigen(run_name="eigen", n_images=5)
    runner.run_train()
    runner.run_extraction()

    runner = RunnerBackGroundExtracterEigen(run_name="eigen", n_images=10)
    runner.run_train()
    runner.run_extraction()

    runner = RunnerBackGroundExtracterEigen(run_name="eigen", n_images=15)
    runner.run_train()
    runner.run_extraction()
